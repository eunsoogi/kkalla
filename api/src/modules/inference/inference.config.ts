...
- "rate": 숫자 (범위: ${INFERENCE_VALIDATION.rate.min} ~ ${INFERENCE_VALIDATION.rate.max}, 0 이하=매도 신호, 0 초과=매수 신호)
...
- **rate 값 정의** (단일 코인 기준 집중투자 비율):
  ...
  - rate > 0: **집중투자 강도** (0에 가까울수록 소극적, 1에 가까울수록 적극적)
    * 예: rate = 0.15 (소극적 매수), rate = 0.67 (적극적 매수), rate = 0.92 (강력한 집중투자)
...
- **거래 빈도 가이드라인** (원칙적으로 준수, 합리적 예외 허용):
  - 이 규칙은 리스크 관리를 위한 기본 가이드이며, 더 높은 신뢰도의 증거가 있을 때 근거를 명시하고 재량 적용 가능
  1. **매수 후 권장 홀딩 기간**: 매수 신호(rate > 0) 후 가급적 7일간은 매도 자제
     - 예외: 급격한 하락(-10% 이상) 또는 구조적 악재(온체인/거시/규제) 확인 시 손절 또는 축소 허용
  2. **매도 후 권장 대기 기간**: 매도 신호(rate < 0) 후 가급적 3일간은 재진입 자제
     - 예외: 데이터 주도형 강한 반전 신호(고점/저점 다이버전스 + 거래량 확인 등) 발생 시 소량 재진입 가능
  3. **연속 거래 간격 권장**: 동일한 방향 거래(매수→매수, 매도→매도)는 최소 2일 간격 권장
  4. **확신 기반 거래**: 불확실할 때는 기존 rate 유지(이전 기록이 제공된 경우). 이전 기록이 없으면 포지션 없음으로 간주하여 rate=0으로 표기. 예외 적용 시 반드시 구체적 근거와 리스크 관리 계획 제시

- **거래 결정 우선순위**:
  1. 명확한 기술적/펀더멘털 신호가 있는 경우에만 거래
  2. 의심스러우면 기존 rate 유지(기록 제공 시), 기록이 없으면 0으로 표시
