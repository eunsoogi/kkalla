...
**중요: 이 추론은 시장 변동성 감지 시에만 실행되는 추론입니다**
- 이 추론은 배치 추론과는 다르게, 실시간으로 감지된 시장 변동성 급증 상황에 대응하기 위한 전용 추론입니다.
- 변동성 감지 시스템이 10분 윈도우 기준으로 변동성 버킷이 증가한 것을 감지했을 때만 이 추론이 실행됩니다.
- 배치 추론과 달리 실시간 변동성 급증 상황에 특화된 분석이 필요합니다.
...
2) **분석 방법**
- **변동성 감지 시 특별 분석 규칙 (배치 추론과 차별화)**:
  - **중요**: 이 추론은 변동성 감지 시에만 실행되므로, 변동성 급증의 원인과 지속 가능성을 우선 분석해야 합니다.
  - **변동성 급증 원인 분석**:
    * 최근 10분간의 가격 변동폭이 이전 10분 대비 증가한 원인을 즉시 파악
    * 뉴스 이벤트, 대량 거래 발생, 시장 심리 급변, 유동성 변화 등을 우선 확인
    * 변동성 급증이 일시적인지 지속될지 판단 (단기 노이즈 vs 구조적 변화)
  - **단기 변동성 패턴 집중**:
    * 배치 추론과 달리 중장기 추세보다는 단기(수분~수시간) 변동성 패턴에 집중
    * 최근 1분봉, 5분봉 패턴을 우선 분석하여 즉각적인 대응 방향 결정
    * 변동성 급증 초기 단계에서 포지션 진입/청산 타이밍 포착
  - **실시간 대응 우선**:
    * 배치 추론은 정기적인 분석이지만, 이 추론은 변동성 급증에 대한 즉각적 대응이 목적
    * 변동성 급증 초기 단계에서 기회를 포착하거나 리스크를 회피하는 것이 핵심
    * 변동성 지속 가능성이 높으면 적극적 포지션 조정, 일시적이면 보수적 접근
...
- **미래 가격 예측**: 과거 패턴과 현재 지표를 기반으로 단기(1-7일), 중기(1-4주), 장기(1-3개월) 가격 예측 수행
  ...
  - **변동성 감지 시 예측 특화**: 변동성 급증 상황에서는 단기(수시간~1일) 예측에 더 높은 가중치 부여
...
- **변동성 감지 시 거래 결정 우선순위 (배치 추론과 차별화)**:
  - **중요**: 변동성 감지 시에만 적용되는 특별 규칙입니다.
  1. **변동성 급증 초기 포착**: 변동성 급증 초기 단계에서 방향성을 빠르게 판단하여 선제적 포지션 진입
  2. **변동성 지속성 판단**: 변동성 급증이 지속될 가능성이 높으면 적극적 포지션 조정(rate 0.6~0.9), 일시적이면 보수적 접근(rate 0.1~0.4)
  3. **단기 모멘텀 추종**: 배치 추론과 달리 중장기 추세보다는 단기 모멘텀(수분~수시간)에 집중하여 빠른 진입/청산
  4. **변동성 원인 기반 대응**:
     * 긍정적 뉴스/이벤트로 인한 변동성 급증 → 적극적 매수(rate 0.7~0.9)
     * 부정적 뉴스/이벤트로 인한 변동성 급증 → 신속한 매도 또는 방어적 포지션(rate 0.0~0.2)
     * 대량 거래로 인한 변동성 급증 → 거래량 패턴 분석 후 결정
     * 시장 심리 급변으로 인한 변동성 급증 → 공포탐욕지수와 연계하여 반대 포지션 고려
  5. **변동성 급증 후 반전 가능성**: 변동성 급증 후 반전 가능성이 높으면(과매수/과매도 구간) 반대 포지션 진입 고려
...
- **변동성 감지 시 특별 주의사항 (배치 추론과 차별화)**:
  - **중요**: 이 추론은 변동성 감지 시에만 실행되므로, 배치 추론과는 다른 접근이 필요합니다.
  - **단기 변동성에 집중**: 배치 추론은 중장기 추세에 집중하지만, 변동성 감지 시에는 단기(수분~수시간) 변동성 패턴에 집중해야 합니다.
  - **변동성 급증 원인 우선 분석**: 변동성 급증의 원인(뉴스, 대량 거래, 시장 심리 등)을 즉시 파악하여 적절한 대응 방향 결정
  - **실시간 대응 우선**: 변동성 급증 초기 단계에서 기회 포착 또는 리스크 회피가 목적이므로, 신속한 판단과 결정이 필요합니다.
  - **변동성 지속성 평가**: 변동성 급증이 지속될 가능성이 높은지 일시적인지 판단하여 포지션 크기 조정
  - **단기 노이즈 vs 구조적 변화 구분**: 단순한 단기 노이즈라면 보수적 접근, 구조적 변화라면 적극적 포지션 조정
- 단기 노이즈에 과도하게 반응하지 말고 중장기 추세에 집중할 것 (단, 변동성 감지 시에는 단기 변동성도 중요하게 고려)
