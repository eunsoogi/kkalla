...
const SlackProgressPanel: React.FC = () => {
  ...
  const checklistItems: ChecklistItem[] = [
    {
      label: t('notify.registerFlow.checklistToken'),
      detail: isSlackRegistered ? t('status.registered') : t('status.unknown'),
      complete: isSlackRegistered,
    },
    {
      label: t('notify.registerFlow.checklistChannel'),
      detail: hasChannel ? t('notify.registerFlow.channelSet') : t('notify.registerFlow.channelMissing'),
      complete: hasChannel,
    },
  ];
  ...
  const nextAction = !isSlackRegistered
    ? t('notify.registerFlow.nextToken')
    : !hasChannel
      ? t('notify.registerFlow.nextChannel')
      : t('notify.registerFlow.nextComplete');
  ...
  return (
    <aside className='rounded-xl shadow-md dark:shadow-dark-md bg-white dark:bg-dark p-6'>
      <div className='flex items-start justify-between gap-3 flex-nowrap'>
        <div>
          <h3 className='text-base font-semibold text-dark dark:text-white'>{t('notify.registerFlow.progressTitle')}</h3>
          <p className='mt-2 text-sm text-gray-500 dark:text-gray-400'>{t('notify.registerFlow.progressDescription')}</p>
        </div>
        ...
        {checklistItems.map((item) => (
          ...
          >
            ...
            <Badge className='shrink-0 whitespace-nowrap' color={item.complete ? 'success' : 'gray'}>
              {item.complete ? t('notify.registerFlow.checklistDone') : t('notify.registerFlow.checklistPending')}
            </Badge>
          </div>
        ))}
      </div>
      ...
      <div className='mt-6 rounded-lg border border-lightprimary bg-lightprimary dark:bg-primary dark:border-primary p-4'>
        <p className='text-xs font-semibold uppercase tracking-wide text-primary dark:text-white'>
          {t('notify.registerFlow.nextAction')}
        </p>
