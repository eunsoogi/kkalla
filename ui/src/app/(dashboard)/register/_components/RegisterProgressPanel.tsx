...
const RegisterProgressPanel: React.FC = () => {
  ...
  const checklistItems: ChecklistItem[] = [
    {
      label: t('service.registerFlow.checklistUpbit'),
      detail: isUpbitRegistered ? t('status.registered') : t('status.unknown'),
      complete: isUpbitRegistered,
    },
    {
      label: t('service.registerFlow.checklistCategories'),
      detail: hasCategory
    ...
    {
      label: t('service.registerFlow.checklistSchedule'),
      detail: isServiceEnabled ? t('service.registerFlow.scheduleOn') : t('service.registerFlow.scheduleOff'),
  ...
  const nextAction = !isUpbitRegistered
    ? t('service.registerFlow.nextUpbit')
    : !hasCategory
      ? t('service.registerFlow.nextCategories')
      : !isServiceEnabled
        ? t('service.registerFlow.nextSchedule')
        : t('service.registerFlow.nextComplete');
  ...
  return (
    <aside className='rounded-xl shadow-md dark:shadow-dark-md bg-white dark:bg-dark p-6'>
      <div className='flex items-start justify-between gap-3 flex-nowrap'>
        <div>
          <h3 className='text-base font-semibold text-dark dark:text-white'>{t('service.registerFlow.progressTitle')}</h3>
          <p className='mt-2 text-sm text-gray-500 dark:text-gray-400'>{t('service.registerFlow.progressDescription')}</p>
        </div>
        ...
        {checklistItems.map((item) => (
          ...
          >
            ...
            <Badge className='shrink-0 whitespace-nowrap' color={item.complete ? 'success' : 'gray'}>
              {item.complete ? t('service.registerFlow.checklistDone') : t('service.registerFlow.checklistPending')}
            </Badge>
          </div>
        ))}
      </div>
      ...
      <div className='mt-6 rounded-lg border border-lightprimary bg-lightprimary dark:bg-primary dark:border-primary p-4'>
        <p className='text-xs font-semibold uppercase tracking-wide text-primary dark:text-white'>
          {t('service.registerFlow.nextAction')}
        </p>
